...
def fn_var_frq_sine_wave(A, y_shift, f_start, f_end, interval, n_steps):
...
class SIRSTimeCompressProcess(Process):
    def __init__(self):
        super().__init__('sir-time-compress-proc', TimeAlways())
        self.wave = fn_var_frq_sine_wave(1, 0, 1, (n_iter / 1000) + 3, 1, n_iter + 1)
        self.last_applied_iter = None
    ...
    def apply(self, pop, group, iter, t):
        if self.last_applied_iter != iter:
            ...
            global sir_beta, sir_gamma
            sir_beta  = min(1.00, sir_beta  + 0.10)
            sir_gamma = min(1.00, sir_gamma + 0.05)
            self.last_applied_iter = iter
...
class RecurrentFluGammaProcess(Process):
    def __init__(self, t=TimeAlways()):
        ...
        self.wave = fn_var_frq_sine_wave(1, 0, 1, n_iter / 1000, 1, n_iter + 1)
        self.last_applied_iter = None
...
# te.plot_mass_locus_line_aggr((1200,600), get_out_dir('_plot.png'), iter_range=(-1, -1), do_sort=True)
