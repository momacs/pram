...
class ProgressAndTransmitFluRule(Rule):
    ...
    def apply(self, pop, group, iter, t):
        ...
        p = 0.01  # prob of infection

        if group.has_rel({ Site.AT: group.get_rel('school') }):
            site = group.get_rel(Site.AT)

            nn = site.get_pop_size(GroupQry(attr={ 'flu-stage': AttrFluStage.NO     }))
...
fpath_db = os.path.join(dir, 'probes.sqlite3')

probe_persist = ProbePersistanceDB(fpath_db, do_overwrite=True)
...
for s in specs:
    ...
    p = GroupSizeProbe(f'flu-{s.name}', [
        ...
        ['pn', 'pa', 'ps', 'nn', 'na', 'ns'],
        GroupQry(rel={ 'school': site }), probe_persist, memo=f'Flu at school {s.name.upper()}'
    )
    probes_grp_size_flu_school.append(p)
